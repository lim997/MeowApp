...
# 담당 역할 (팀원: 조예림)
- 애플리케이션 제작 기여도 100%
- JAVA 와 XML 을 사용하여, 밥그릇과 연동되는 애플리케이션 제작.
- SQLite 를 이용해 데이터베이스를 구축하여 사진의 경로를 저장하고, 배식 시간을 설정.
- URL 통신을 이용하여 데이터를 GET/ POST.
- 서버와의 통신으로 카메라 속 반려동물의 모습을 실시간으로 출력, 설정된 배식 시간에 자동으로 음식을 배분, 사용자가 선택한 음악을 스피커로 출력.
